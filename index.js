...
console.log(color(`${spc2}           [ CREATOR BY: SAN MOTZ ]` ,`${randomcolor}`))
...
console.log(color(`${spc3}                    [•]`, `${randomcolor}`), color(`Base Ori By : San Motz`, `${randomcolor}`))
console.log(color(`${spc4}                       < =============================== >`, `${randomcolor}`))
    let { version, isLatest } = await fetchLatestBaileysVersion()
    const Danzz = DanzzConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['San Motz','Chrome','3.0.0'],
        auth: state,
        version
    })
